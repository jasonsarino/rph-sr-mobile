<table class="table datatable-divisions table-striped table-hover">
	<thead>
		<tr class="bg-success">
            <th>DIVISION NAME</th>
            <th>ACTIONS</th>
	...
	<tbody>
		...
		<tr class="<?= $bg?>">
			...
			<td>
				<?php 
				if( (isset($user_privileges['edit.manage_divisions']) AND $user_privileges['edit.manage_divisions'] == 1) OR $user_privileges == 'FULL-ACCESS' ) {
				...
				if( (isset($user_privileges['delete.manage_divisions']) AND $user_privileges['delete.manage_divisions'] == 1) OR $user_privileges == 'FULL-ACCESS' ) {
...
$('.datatable-divisions').dataTable({
    ...
    buttons: [
        ...
        {
            text: '<i class="icon-plus3"></i> Add New Division',
            className: 'btn btn-bg-primary marginleft <?=$hideClassName?>',
            action: function(e, dt, node, config) {
            	location.href=base_url+'manage_divisions/add';
            }
        }
    ]
});
